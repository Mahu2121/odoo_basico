...
######
     ...
     def action_crear_factura(self):
         """Crear una factura de cliente (account.move) con una línea basada en este registro.

         - Partner: partner comercial del usuario actual.
         - Producto: usa un producto genérico si existe, o crea uno mínimo.
         - Precio: usa el campo peso como cantidad y volume como precio unitario (ejemplo).
         """
         self.ensure_one()

         # Asegurarse de que el módulo de facturación está instalado
         Move = self.env['account.move'].sudo()

         # Determinar el partner: usamos el partner del usuario actual
         partner = self.env.user.partner_id
         if not partner:
             raise ValidationError('El usuario actual no tiene un contacto asociado para facturar.')

         # Buscar un producto genérico o crear uno mínimo
         Product = self.env['product.product'].sudo()
         product = Product.search([('default_code', '=', 'INFO_ODBAS')], limit=1)
         if not product:
             product = Product.create({
                 'name': 'Servicio información odoo_basico',
                 'default_code': 'INFO_ODBAS',
                 'list_price': 1.0,
                 'type': 'service',
             })

         # Datos de la línea: de ejemplo usamos peso como cantidad y volume como precio unitario
         quantity = self.peso or 1.0
         price_unit = self.volume or 1.0
         ...
         move_vals = {
             ...
             'invoice_line_ids': [
                 (0, 0, {
                     ...
                     'name': self.descripcion or self.name,
                     'quantity': quantity,
         ...
         # Abrir la factura creada en vista formulario
